...
namespace
{
    ...
    float sampleTerrainHeightBilinear(const elix::engine::TerrainAsset &terrainAsset,
                                      float localX,
                                      float localZ)
    {
                                    ...
                                    glm::vec3 &outHitLocal)
    {
        ...
        for (int stepIndex = 1; stepIndex <= maxSteps; ++stepIndex)
        {
            ...
            if (crossedSurface)
            {
                ...
                outHitLocal = rayOriginLocal + rayDirectionLocal * ((lowT + highT) * 0.5f);
                return true;
            }
...
namespace elix
{
    namespace plugin
    {
        namespace terrain
        {
            ...
            bool TerrainTools::draw(bool *open)
            {
                ...
                if (ImGui::Button("Spawn Terrain Entity"))
                {
                    ...
                    if (spawnTerrainEntityFromAsset(m_spawnTerrainAssetPath, error))
                        ...
                        VX_EDITOR_ERROR_STREAM("Failed to spawn terrain entity: " << error << '\n');
                }

                ImGui::Spacing();
                ImGui::SeparatorText("Brush");
                static const char *kBrushModes[] = {"Raise", "Lower", "Smooth", "Flatten", "Paint Layer"};
                                                       ...
                                                       bool strokeStart)
            {
                ...
                if (!terrainComponent)
                {
                    for (const auto &entity : m_scene->getEntities())
                    {
                        ...
                        auto *candidate = entity->getComponent<engine::TerrainComponent>();
                        if (!candidate)
                            continue;
                ...
                if (m_brushSettings.mode == BrushMode::PaintLayer && terrainAsset.weightmapData.empty())
                {
                    ...
                    terrainAsset.weightmapChannels = std::clamp<uint32_t>(
                        std::max<uint32_t>(1u, static_cast<uint32_t>(terrainAsset.layers.size())), 1u, 4u);
                    terrainAsset.weightmapData.assign(
                        static_cast<size_t>(terrainAsset.weightmapWidth) *
                ...
                for (int sampleY = sampleMinY; sampleY <= sampleMaxY; ++sampleY)
                {
                    for (int sampleX = sampleMinX; sampleX <= sampleMaxX; ++sampleX)
                    {
                        ...
                        const float brushWeight = computeBrushWeight(distance / brushRadius, m_brushSettings.falloff, m_brushSettings.useWorldSpaceFalloff);
                        ...
                        if (m_brushSettings.mode == BrushMode::PaintLayer)
                        {
                            ...
                            for (uint32_t ch = 0; ch < terrainAsset.weightmapChannels; ++ch)
                                channels[ch] = static_cast<float>(terrainAsset.weightmapData[baseIndex + ch]);
                            ...
                            if (terrainAsset.weightmapChannels > 1u)
                            {
                                ...
                                for (uint32_t ch = 0; ch < terrainAsset.weightmapChannels; ++ch)
                                {
                                    if (static_cast<int>(ch) == activeLayer)
                                        continue;
                                    channels[ch] = std::clamp(channels[ch] - bleed, 0.0f, 255.0f);
                                }
                            }
                            ...
                            if (m_brushAutoNormalizeWeights)
                            {
                                ...
                                if (sum > kEpsilon)
                                {
                                    ...
                                    for (uint32_t ch = 0; ch < terrainAsset.weightmapChannels; ++ch)
                                        channels[ch] = std::clamp(channels[ch] * scale, 0.0f, 255.0f);
                                }
                                else
                                {
                                    for (uint32_t ch = 0; ch < terrainAsset.weightmapChannels; ++ch)
                            ...
                            for (uint32_t ch = 0; ch < terrainAsset.weightmapChannels; ++ch)
                            {
                                const uint8_t val = static_cast<uint8_t>(std::clamp(std::round(channels[ch]), 0.0f, 255.0f));
                                if (terrainAsset.weightmapData[baseIndex + ch] != val)
                                    terrainAsset.weightmapData[baseIndex + ch] = val;
                            }
                            continue;
                        }
                        ...
                        case BrushMode::Smooth:
                        {
                            ...
                            for (int oy = -1; oy <= 1; ++oy)
                            {
                                ...
                                {
                                    const int nx = std::clamp(sampleX + ox, 0, static_cast<int>(terrainAsset.width) - 1);
                                    const int ny = std::clamp(sampleY + oy, 0, static_cast<int>(terrainAsset.height) - 1);
                                    const size_t ni = static_cast<size_t>(ny) * static_cast<size_t>(terrainAsset.width) + static_cast<size_t>(nx);
                                    if (ni >= smoothSourceHeights.size())
                                        continue;
                                    neighborSum += static_cast<float>(smoothSourceHeights[ni]) / 65535.0f;
                                    ++neighborCount;
                                }
                            }
                            if (neighborCount > 0)
                            {
                                const float avg = neighborSum / static_cast<float>(neighborCount);
                                const float blend = std::clamp(sculptRate * 18.0f, 0.0f, 1.0f);
                                newHeightNormalized = glm::mix(currentHeightNormalized, avg, blend);
                            }
                            break;
                        }
                        ...
                        {
                            const float targetNorm = std::clamp(m_flattenTargetWorldHeight / std::max(terrainAsset.heightScale, 0.01f), 0.0f, 1.0f);
                            const float blend = std::clamp(sculptRate * 20.0f, 0.0f, 1.0f);
                            newHeightNormalized = glm::mix(currentHeightNormalized, targetNorm, blend);
                            break;
                        }
                        ...
                        const uint16_t newSample = static_cast<uint16_t>(std::clamp(std::round(newHeightNormalized * 65535.0f), 0.0f, 65535.0f));
                        if (terrainAsset.heightSamples[sampleIndex] != newSample)
                        {
                            terrainAsset.heightSamples[sampleIndex] = newSample;
            ...
            bool TerrainTools::createFlatTerrainAsset(std::string &outAssetPath, std::string &outError) const
            {
                ...
                terrainAsset.heightSamples.assign(
                    static_cast<size_t>(terrainAsset.width) * static_cast<size_t>(terrainAsset.height), 0u);
                ...
                const std::filesystem::path outputPath =
                    (m_projectRootPath / "resources" / "terrains" / (terrainName + ".elixterrain")).lexically_normal();
